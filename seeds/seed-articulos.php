...
$totalCategorias = ejecutarConsultaSimpleFila("SELECT COUNT(*) AS total FROM categoria");

if (!$totalCategorias || (int)$totalCategorias['total'] === 0) {
    echo "ERROR: La tabla 'categoria' está vacía.\n";
    echo "Ejecutá primero: php seed-categorias.php\n";
    exit(1);
}

echo "Categorias encontradas: {$totalCategorias['total']}\n";
echo "---------------------------------\n";
...
foreach ($articulos as $art) {
    ...
    $imagen      = $art['codigo'] . '-' . $art['nombre'];   // ej: "ART-001-Mouse Logitech G502 Hero"

    // ── ¿Ya existe? → se elimina para volver a insertar
    $existe = ejecutarConsultaSimpleFila(
        "SELECT idarticulo FROM articulo WHERE nombre = '$nombre'"
    );

    if ($existe) {
        ejecutarConsulta("DELETE FROM articulo WHERE nombre = '$nombre'");
    }

    // ── Insertar (nuevo o reemplazado)
    $sql = "INSERT INTO articulo (idcategoria, codigo, nombre, stock, descripcion, imagen, condicion)
            VALUES ('{$art['idcategoria']}', '$codigo', '$nombre', '{$art['stock']}', '$descripcion', '$imagen', 1)";

    $resultado = ejecutarConsulta($sql);

    if ($resultado) {
        if ($existe) {
            echo "Reemplazado : {$art['nombre']}\n";
            $reemplazados++;
        } else {
